...
struct batch_normalization_test_params_t {
    dt src_dt;
...
bool cuda_check_format_tag(tag atag) {
    return impl::utils::one_of(atag, tag::ncdhw, tag::ndhwc, tag::nchw,
...
private:
    batch_normalization_test_params_t p;
    memory src, workspace, mean, variance, scale;
    std::shared_ptr<batch_normalization_forward::primitive_desc> pd_fwd_hint;

protected:
    void SetUp() override {
        p = ::testing::TestWithParam<
                batch_normalization_test_params_t>::GetParam();
    ...
    void Forward(prop_kind pk, normalization_flags flags) {
        ...
        auto pd = pd_t();
        // regular pd ctor
        pd = pd_t(eng, pk, src_md, dst_md, /* epsilon = */ 1e-4f, flags);
        // test all pd ctors
        test_fwd_pd_constructors<pd_t>(pd, aa, pk, src_md, dst_md,
                /* epsilon = */ 1e-4f, flags);
        pd_fwd_hint = std::make_shared<pd_t>(pd);

        EXPECT_ANY_THROW(batch_normalization_forward(pd, {}));
        // default primitive ctor
        auto batch_normalization = batch_normalization_forward();
        // regular primitive ctor
        batch_normalization = batch_normalization_forward(pd);

        // check primitive kind is batch_normalization
        ASSERT_TRUE(batch_normalization.get_kind()
                == primitive::kind::batch_normalization);
        ...
        auto shift = test::make_memory(shift_desc, eng);
        ...
        // test out-place mode
        batch_normalization.execute(strm,
    ...
    void Backward(prop_kind pk, normalization_flags flags) {
        ...
        auto pd = pd_t();
        // regular pd ctor
        pd = pd_t(eng, pk, diff_src_md, diff_dst_md, src_md,
                /* epsilon = */ 1e-4f, flags, *pd_fwd_hint);
        // test all pd ctors
        test_bwd_pd_constructors<pd_t, hint_pd_t>(pd, *pd_fwd_hint, aa, pk,
                diff_src_md, diff_dst_md, src_md,
                /* epsilon = */ 1e-4f, flags);

        EXPECT_ANY_THROW(batch_normalization_backward(pd, {}));
        // default primitive ctor
        auto batch_normalization = batch_normalization_backward();
        // regular primitive ctor
        batch_normalization = batch_normalization_backward(pd);
    ...
    void Test() {
        ...
        for (auto flags : inference_flags) {
            SKIP_FOR_LOOP(p.src_dt == dt::s8 && !use_global_stats(flags),
        ...
        std::vector<normalization_flags> training_flags {nf::none,
                nf::fuse_norm_relu, nf::use_scale, nf::use_shift,
                nf::fuse_norm_relu | nf::use_scale,
                nf::fuse_norm_relu | nf::use_shift,
                nf::use_scale | nf::use_shift,
                nf::fuse_norm_relu | nf::use_scale | nf::use_shift};
...
using tp = batch_normalization_test_params_t;

TEST_P(batch_normalization_test_t, TestsBatchNormalization) {}

INSTANTIATE_TEST_SUITE_P(Test_BatchNormalization_EF, batch_normalization_test_t,
        ::testing::Values(
                // Negative dims
                tp {dt::f32, dt::f32, dt::undef, tag::nchw, tag::nchw,
                        tag::undef, {2, -2, 128, 256}, true,
                ...
                tp {dt::f32, dt::f32, dt::f32, tag::any, tag::nchw, tag::nchw,
                        {2, 2, 128, 256}, true, dnnl_invalid_arguments},
                ...
                tp {dt::f32, dt::bf16, dt::f32, tag::nchw, tag::nchw, tag::nchw,
                        {2, 2, 128, 256}, true, dnnl_unimplemented},
...
#define INST_TEST_CASE(name, suite, ...) \
    INSTANTIATE_TEST_SUITE_P( \
            name, batch_normalization_test_t, suite(__VA_ARGS__));
...
INST_TEST_CASE(
        BatchNormalizationSimpleF32, all_cases, EXPAND_DTS(f32, f32, f32));
INST_TEST_CASE(
        BatchNormalizationSimpleBF16, all_cases, EXPAND_DTS(bf16, bf16, bf16));
INST_TEST_CASE(
        BatchNormalizationSimpleF16, all_cases, EXPAND_DTS(f16, f16, undef));
INST_TEST_CASE(
        BatchNormalizationSimpleS8, all_cases, EXPAND_DTS(s8, s8, undef));
} // namespace dnnl
