...
cell_execution_sig((simple_common_t<aprop>::cell_execution_gru)) {
    ...
    if (aprop == prop_kind::forward) {
        ...
        // 2. gemm Wh[0-1],h
        CHECK(matmul_primitive(engine, ctx, cell_iter, wei_iter, scratch_gates,
                matmul_iter_fwd));
        ...
        // 4. gemm Wh[2],h~t
        CHECK(matmul_primitive(engine, ctx, cell_iter2,
                {wei_iter, cell_wei_iter_offset2},
                {scratch_gates, cell_scratch_offset2}, matmul_iter_fwd_2));
        ...
        // d(hG1) = dG2 * W2h^t
        CHECK(matmul_primitive(engine, ctx,
        ...
        CHECK(matmul_primitive(engine, ctx, cell_iter, diff_gates,
                diff_wei_iter, matmul_diff_wei_iter));

        CHECK(matmul_primitive(engine, ctx, scratch_cell,
                {diff_gates, cell_scratch_diff_off2},
        ...
        CHECK(matmul_primitive(engine, ctx, diff_gates, wei_iter, diff_states,
                matmul_iter_bwd));
